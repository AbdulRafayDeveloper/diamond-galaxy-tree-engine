"use client";
const PropertyTable = ({ PropertyProducts }) => {
  // const getStatusColor = (status) => {
  //   if (status === "Active") return "text-green-500 font-medium bg-[#F0FDF4]";
  //   if (status === "Pending") return "text-yellow-400 font-medium bg-[#FCFDF0] ";
  //   if (status === "Expire") return "text-red-400 font-medium bg-[#FDF0F0] ";
  ...
  return (
    <div className="relative overflow-x-auto scrollbar-none pt-12 ">
      <table className="w-full text-sm text-left text-gray-500 p-3">
        ...
        <tbody>
          {PropertyProducts && PropertyProducts.map((product, index) => (
            <tr
              key={index}
              className="odd:bg-[#F5F5F5] even:bg-white border-b border-gray-200 text-center "
            >
              <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap flex justify-center items-center gap-3">
                <img
                  src={product.image}
                  alt={product.name}
