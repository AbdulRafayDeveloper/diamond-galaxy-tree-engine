...
//         const winnerUser = users.find((user) => user.name === winner);
//         setWinnerDetails(winnerUser);
//         setShowModal(true);
//     };
...
//     const section = 'Lucky Draw';
//     const segments = users.map((user) => user.name);
//     const segColors = ['#22405c', '#F6F1DE', '#22405c', '#F6F1DE', '#22405c', '#F6F1DE'];
...
//                         </svg>
//                     </button>
//                     <p className="text-[12px] md:text-xl md:font-semibold ml-4 md:ml-64 lg:ml-64 p-5">Lucky Draw</p>
//                     <div className="ml-auto">
//                         <Header appear={true} />
//                     </div>
//                 </div>
...
//                                         </p>
//                                         <p>
//                                             <strong>Registered:</strong> {winnerDetails.isRegistered ? 'Yes' : 'No'}
//                                         </p>
//                                         <p>
//                                             <strong>Activated:</strong> {winnerDetails.isActivated ? 'Yes' : 'No'}
//                                         </p>
//                                         <p>
...
//                                     </div>
//                                     <Link href="../../../admin/lucky-draw">
//                                         <button onClick={closeModal} className="mt-4 bg-[#22405c] text-white px-4 py-2 rounded">
//                                             Close
//                                         </button>
//                                     </Link>
//                                 </div>
...
//                                     />
//                                     <div className="flex justify-end gap-2">
//                                         <button className="bg-[#F6F1DE] px-4 py-2 rounded" onClick={closeModalAmount}>
//                                             Cancel
//                                         </button>
//                                         <button className="bg-[#22405c] text-white px-4 py-2 rounded" onClick={handleSubmit}>
//                                             Submit
//                                         </button>
//                                     </div>
//                                 </div>
...
// Colors for each segment (alternating)
const segmentColors = ['#22405c', '#F6F1DE'];

function CustomWheel({ segments, onFinish }) {
  const [spinning, setSpinning] = useState(false);
  const [rotation, setRotation] = useState(0);
  const wheelRef = useRef(null);

  // Number of segments and degrees per segment
  ...
  const spinWheel = () => {
    ...
    // Randomly pick a segment index as the winner
    const winnerIndex = Math.floor(Math.random() * segmentCount);

    // Calculate how many full rotations + angle to stop at winner segment
    // Add extra full spins for effect (e.g., 5 full spins)
    const extraSpins = 5;
    const stopAngle = 360 * extraSpins + (360 - winnerIndex * degreesPerSegment - degreesPerSegment / 2);

    // Animate the rotation with CSS transition
    setRotation((prev) => prev + stopAngle);

    // After animation ends (5s), notify parent with winner segment
    setTimeout(() => {
      setSpinning(false);
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        ref={wheelRef}
        className="relative rounded-full border-4 border-[#22405c] overflow-hidden"
        style={{
          width: 290,
          height: 290,
      ...
      >
        {/* Render segments as colored slices */}
        {segments.map((segment, i) => {
  const rotate = i * degreesPerSegment;
  return (
    <div
      key={i}
      className="absolute top-0 left-0 w-full h-full origin-center"
      style={{
        transform: `rotate(${rotate}deg) skewY(-${90 - degreesPerSegment}deg)`,
    ...
    >
      <span
        style={{
          transform: `skewY(${90 - degreesPerSegment}deg) rotate(${-rotate - degreesPerSegment / 2}deg)`,
          color: i % 2 === 0 ? 'black' : 'white', // Light text on dark bg, dark text on light bg
          fontWeight: 'bold',
          fontSize: 14,
      ...
      <button
        disabled={spinning}
        onClick={spinWheel}
        className="mt-6 px-6 py-2 rounded bg-[#22405c] text-white font-semibold hover:bg-[#163348] disabled:opacity-50"
      >
      ...
      <div
        style={{
          ...
          borderBottom: '30px solid #e3342f',
          left: '50%',
      ...
      <div className="sm:ml-64">
        <div className="p-2 bg-white">
          <div className="flex flex-col items-center gap-4 mt-3 relative">
            <CustomWheel segments={segments} onFinish={handleSpinFinish} />
              ...
              <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg p-6 w-80 shadow-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <button className="bg-[#22405c] text-white px-4 py-2 rounded mt-4" onClick={() => setIsOpen(true)}>
                      Add Deposit
                    </button>
                  </div>
                  <Link href="../../../admin/lucky-draw">
